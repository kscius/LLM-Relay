...
export class XAIAdapter extends BaseProviderAdapter {
  ...
  readonly capabilities: ProviderCapabilities = {
    ...
    availableModels: ['grok-3-beta', 'grok-3-fast-beta', 'grok-3-mini-beta', 'grok-3-mini-fast-beta', 'grok-2-latest', 'grok-2-vision-latest'],
  };
  ...
  normalizeError(err: unknown, status?: number): NormalizedError {
    if (err instanceof OpenAI.APIError) {
      if (err.status === 401) return { type: 'auth', message: 'Invalid xAI API key' };
      if (err.status === 402) return { type: 'billing', message: 'xAI: add credits' };
      if (err.status === 429) {
