...
export class OpenAIAdapter extends BaseProviderAdapter {
  ...
  readonly capabilities: ProviderCapabilities = {
    ...
    availableModels: ['gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo', 'gpt-4', 'gpt-3.5-turbo', 'o1-preview', 'o1-mini'],
  };
  ...
  normalizeError(err: unknown, status?: number): NormalizedError {
    if (err instanceof OpenAI.APIError) {
      if (err.status === 401) return { type: 'auth', message: 'Invalid OpenAI API key' };
      if (err.status === 402) return { type: 'billing', message: 'OpenAI billing issue - check payment' };
      if (err.status === 429) {
