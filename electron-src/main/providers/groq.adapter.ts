...
export class GroqAdapter extends BaseProviderAdapter {
  ...
  readonly capabilities: ProviderCapabilities = {
    ...
    availableModels: ['llama-3.3-70b-versatile', 'llama-3.1-8b-instant', 'llama-3.2-3b-preview', 'mixtral-8x7b-32768', 'gemma2-9b-it'],
  };
  ...
  normalizeError(err: unknown, status?: number): NormalizedError {
    if (err instanceof OpenAI.APIError) {
      if (err.status === 401) return { type: 'auth', message: 'Invalid Groq API key' };
      if (err.status === 429) {
