 ...
 * 
 * Cost-effective Chinese AI lab with strong coding capabilities.
 * Uses OpenAI-compatible API. Known for DeepSeek-V3 and DeepSeek-Coder.
 */
export class DeepSeekAdapter extends BaseProviderAdapter {
  ...
  async testConnection(apiKey: string): Promise<ConnectionTestResult> {
    ...
    try {
      // DeepSeek has a models endpoint
  ...
  normalizeError(error: unknown, statusCode?: number): NormalizedError {
    if (error instanceof OpenAI.APIError) {
      ...
      if (status === 401) {
        return { type: 'auth', message: 'Invalid DeepSeek API key' };
      }
      if (status === 402) {
        return { type: 'billing', message: 'DeepSeek billing issue - add credits' };
      }
      if (status === 429) {
